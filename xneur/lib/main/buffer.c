...
static void buffer_clear(struct _buffer *p, struct _xneur_handle *handle)
{
	for (int i = 0; i < p->cur_pos; i++)
	{
		p->keycode[i] = 0;
...
static void append_to_i18n_content(struct _buffer *buf, int language_count, size_t pos, KeyCode keycode, int modifier)
{
	for (int i = 0; i < language_count; i++)
	{
		...
		void *tmp = realloc(p->content, (strlen(p->content) + symbol_len + 1) * sizeof(char));
		assert(tmp != NULL);
		p->content = (char *)tmp;
		p->content = strcat(p->content, symbol);

		size_t symbol_unchanged_len = strlen(symbol_unchanged);
		tmp = realloc(p->content_unchanged, (strlen(p->content_unchanged) + symbol_unchanged_len + 1) * sizeof(char));
		assert(tmp != NULL);
		p->content_unchanged = (char *)tmp;
		p->content_unchanged = strcat(p->content_unchanged, symbol_unchanged);

		tmp = realloc(p->symbol_len, (pos + 1) * sizeof(int));
		assert(tmp != NULL);
		p->symbol_len = (int *)tmp;
		p->symbol_len[pos] = symbol_len;

		tmp = realloc(p->symbol_len_unchanged, (pos + 1) * sizeof(int));
		assert(tmp != NULL);
		p->symbol_len_unchanged = (int *)tmp;
		p->symbol_len_unchanged[pos] = symbol_unchanged_len;
...
static void buffer_set_content(struct _buffer *p, struct _xneur_handle *handle, const char *new_content)
{
	if (new_content == NULL)
		return;
	...
	p->keymap->convert_text_to_ascii(p->keymap, handle, p->content, p->keycode, p->keycode_modifiers);
...
static void buffer_rotate_layout(struct _buffer *p, struct _xneur_handle *handle)
{
	int languages_mask = get_languages_mask();
	...
	for (int i = 0; i < p->cur_pos; i++)
	{
		...
		for (int lang = 0; lang < handle->total_languages; lang++)
		{
			...
			{
				lang++;
				if (lang == handle->total_languages)
					lang = 0;
				int keycode_mod	= get_keycode_mod(lang);
				p->keycode_modifiers[i] = p->keycode_modifiers[i] & languages_mask;
				p->keycode_modifiers[i] = p->keycode_modifiers[i] | keycode_mod;
...
static void buffer_add_symbol(struct _buffer *p, struct _xneur_handle *handle, char sym, KeyCode keycode, int modifier)
{
...
static void buffer_del_symbol(struct _buffer *p, struct _xneur_handle *handle)
{
	if (p->cur_pos == 0)
		return;
	...
	{
		struct _buffer_content *buf = &p->i18n_content[i];
		buf->content          [strlen(buf->content          ) - buf->symbol_len          [p->cur_pos]] = NULLSYM;
		buf->content_unchanged[strlen(buf->content_unchanged) - buf->symbol_len_unchanged[p->cur_pos]] = NULLSYM;
	}
}
...
static char *buffer_get_utf_string_on_kbd_group(struct _buffer *p, struct _xneur_handle *handle, int group)
{
	char *utf_string = (char *) malloc(1 * sizeof(char));
	utf_string[0] = NULLSYM;
	...
	for (int i = 0; i < p->cur_pos; i++)
	{
		...
		for (int j = 0; j < handle->total_languages; j++)
		{
...
static void buffer_uninit(struct _buffer *p, struct _xneur_handle *handle)
{
	if (p == NULL)
		return;
